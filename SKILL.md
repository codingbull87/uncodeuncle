...
name: md2report
description: >
  [LEGACY REPORT SKILL] A4 连续报告模式（不是 16:9 deck）。
  将长篇 Markdown 产业报告重构为咨询风 HTML 报告，并导出分页合理、
  放大仍清晰的 PDF。16:9 deck 请使用 md2deckhtml。
metadata:
  author: zheliu
  updated: 2026-04-20
---
...
- 报告正文是已确认内容，视觉调整通过 `RECOMMENDATIONS.json` 完成；`RECOMMENDATIONS.storyboard.md` 只是派生预览，不是唯一真实源
...
├── FRAGMENT_NORMALIZATION.json
├── LAYOUT_PLAN.json
├── ASSEMBLY_DIAGNOSTICS.json
├── THEME_RESOLUTION.json
├── GATE_STATUS.before-fragments.json
├── GATE_STATUS.before-assemble.json
├── GATE_STATUS.before-export.json
...
├── visual-qa/
│   ├── html-first-page.png
│   ├── html-first-page.normalized.png
│   └── pdf-first-page.png
├── libs/
│   └── echarts.min.js
├── chart-fragments/
...
scripts/
├── build_anchor_index.py
├── prepare_recommendations.py
├── recommendation_loader.py
├── recommendation_state.py
├── report_contract.py
├── insertion_planner.py
├── normalize_fragments.py
├── fragment_sanitizer.py
├── pipeline_common.py
├── theme_resolver.py
├── visual_layout.py
├── assembly_builder.py
...
- `RECOMMENDATIONS.json` 是 recommendation 的唯一真实源；`RECOMMENDATIONS.storyboard.md` 只能由脚本派生，不能替代 JSON 门禁
...
- 进入 Phase 6/7 前必须先生成 `ANCHOR_INDEX.json`、`RECOMMENDATIONS.normalized.json`、`ANCHOR_MATCH_REPORT.json`
...
```bash
python3 {skill_dir}/scripts/build_anchor_index.py {report_dir}
python3 {skill_dir}/scripts/prepare_recommendations.py {report_dir}
python3 {skill_dir}/scripts/check_phase_contract.py {report_dir} before-fragments
python3 {skill_dir}/scripts/normalize_fragments.py {report_dir}
...
python3 {skill_dir}/scripts/qa_layout.py {report_dir}
...
python3 {skill_dir}/scripts/qa_pdf.py {report_dir}/{report_name}_illustrated.pdf
python3 {skill_dir}/scripts/qa_visual.py \
  {report_dir}/{report_name}_illustrated.html \
  {report_dir}/{report_name}_illustrated.pdf \
  {report_dir}/VISUAL_QA.json
```
...
**路由参考：**
- 战略研究/投资判断/咨询交付 → **A. Green** (`green`)
- 长文研究/管理层叙事/解释型报告 → **B. Warm** (`warm`)
- 高层点评/品牌战略/观点表达 → **C. Wine** (`wine`)
- 董事会摘要/文字密集/财务页 → **D. Black** (`black`)
- 企业级/机构级/技术平台报告 → **E. Blue** (`blue`)
...
[A] Green — 麦肯锡系深绿，咨询交付密度
[B] Warm — 暖中性叙事风，阅读友好
[E] Blue — 机构蓝+冷灰，系统化商务风
...
```json
{
  ...
  "color_candidates": ["green", "warm", "blue"]
}
```

如果用户还没有确认，只能写 `color_confirmed: false`，并停止在 Phase 2，不允许继续生成推荐项、片段、HTML 或 PDF。
禁止把 `color_selected_by` 写成 `assistant_default_assumption` 之类默认假设值来绕过门禁；`color_confirmed: true` 时，`color_selected_by` 必须是 `user` 或 `orchestrator-confirmed`。
...
- `RECOMMENDATIONS.json`：唯一真实源，必须由后续脚本继续读取
- `RECOMMENDATIONS.normalized.json`：脚本规范化后的机器版本
- `RECOMMENDATIONS.storyboard.md`：供人类快速浏览的派生预览
- `ANCHOR_MATCH_REPORT.json`：每条 recommendation 的锚点解析结果
...
- `type`：使用下方图表类型
- `anchor_id`：优先使用；引用 `ANCHOR_INDEX.json` 中的锚点 id
- `anchor`：纯标题文本，不带 `#`；脚本会把它解析并回填成 `anchor_id`
...
- `group_anchor` / `row_anchor`：兼容旧字段；脚本会归一化为共享锚点合同
- `row_title`：并排视觉行标题
...
`RECOMMENDATIONS.storyboard.md` 是脚本派生产物，便于人类审阅：
...
组装器读取 `RECOMMENDATIONS.json` 作为唯一真实源。`RECOMMENDATIONS.normalized.json` 和 `RECOMMENDATIONS.storyboard.md` 都只是派生产物；如需调整 recommendation，应修改 `RECOMMENDATIONS.json` 后重新运行 `prepare_recommendations.py`。

`LAYOUT_OVERRIDES.json` 是自动修复产物，不是人工主编辑面。它应由 `repair_layout.py` 每轮重建；需要长期保留的排版意图，应回写到 `RECOMMENDATIONS.json`。
...
- `ANCHOR_INDEX.json`、`RECOMMENDATIONS.normalized.json`、`ANCHOR_MATCH_REPORT.json` 是否已生成且无 unresolved anchor
...
- 读取 `content.html`
- 读取 `RECOMMENDATIONS.json` 作为唯一真实源
...
- 注入固定 CSS 和打印按钮脚本
- 写出 `ASSEMBLY_DIAGNOSTICS.json` 和 `THEME_RESOLUTION.json`
...
`qa_pdf.py` 现在只验证最终导出的 `{report_name}_illustrated.pdf`：检查 PDF 是否可解析、页数是否和 `EXPORT_DIAGNOSTICS.json` 一致、页面尺寸是否一致、页面方向是否异常，以及是否出现完全抽不出文本的可疑页面。它不再伪装成“HTML 代理 PDF QA”。

`qa_visual.py` 会对最终 HTML 与最终 PDF 做轻量视觉回归：渲染 PDF 首页面、触发 HTML 的 print 态首屏截图，再计算 RMSE。它不是像素级审判器，但能抓住“HTML 看着对、PDF 导出后明显走样”的回归。`VISUAL_QA.json` 必须存在；如果环境缺失可用渲染器，它可以 `skipped=true`，此时 `pass` 应为 `null`，不能伪造成 `true` 来冒充做过视觉检查。

`run_pipeline.py` / `run_pipeline_parallel.py` 会把 `LAYOUT_OVERRIDES.json` 当成派生文件，在每次流水线起点删除旧值，并在 layout repair 后重新生成。只要 `LAYOUT_DIAGNOSIS.json.sparsePages` 或 `terminalSparsePages` 仍非空，流水线就必须失败，不能继续导出。

统一入口还会持续写入 `PIPELINE_STATUS.json`，记录当前阶段、已完成阶段、最终产物路径、以及失败原因，并汇总 `PDF_QA.json` 和 `VISUAL_QA.json`。不要再只靠终端输出判断流水线是否真的跑完。
...
优先改 `RECOMMENDATIONS.json`：

- 改 `enabled` 可开关某张图
- 改 `anchor_id`、`anchor`、`anchor_occurrence`、`position` 可调插入位置
...
3. **只改一个真实源**：现在 recommendation 的唯一真实源是 `RECOMMENDATIONS.json`。改完后重新运行 `prepare_recommendations.py`，让脚本重建 `RECOMMENDATIONS.normalized.json` 和 `ANCHOR_MATCH_REPORT.json`。

4. **验证匹配**：修改后用 Python 直接测 `iter_heading_matches(html, anchor)` 确认返回非空，再跑 `assemble.py`。

**重要**：`assemble.py` 在组装（fragment injection）阶段读取 `RECOMMENDATIONS.json`。因此调试 anchor 匹配时不要直接碰派生 storyboard 或 normalized 副本，而是修改 `RECOMMENDATIONS.json` 后重新运行 `prepare_recommendations.py`。
...
- `scripts/fragment_sanitizer.py`：图表片段清洗纯函数，负责去下载残留、修 formatter、清理伪来源
- `scripts/insertion_planner.py`：锚点匹配、段落避让与注入位置规划纯函数
- `scripts/recommendation_loader.py`：recommendation 输入源解析、JSON 读取与旧源诊断；`RECOMMENDATIONS.json` 是唯一真实源
- `scripts/recommendation_state.py`：LAYOUT_OVERRIDES 读取与 recommendation patch 应用逻辑
- `scripts/report_contract.py`：共享 chart id / anchor / layout / page-role 规范化 contract
- `scripts/assembly_builder.py`：cover/meta、fragment 载入和最终 HTML 组装主控
- `scripts/assembly_service.py`：实际组装入口的依赖编排层，供 `assemble.py` 调用
- `scripts/visual_layout.py`：并排 visual row、layout plan 和 visual-block wrapper 生成纯函数
- `scripts/assembly_output.py`：最终 HTML 组装、交付诊断和验证摘要纯函数
- `scripts/smoke_e2e.py`：复制真实工作区并运行安装版 pipeline 的端到端 smoke 校验
- `scripts/lint_fragments.py`：图表片段质量红线检查
- `scripts/qa_html.py`：最终 HTML 结构、并排组、非法残留和协议漂移检查
- `scripts/qa_pdf.py`：最终交付 PDF 的结构化验证
- `scripts/qa_visual.py`：最终 HTML 与最终 PDF 的首页面轻量视觉回归 QA
...
- `scripts/pipeline_common.py`：串行/并行入口共享的状态与门禁辅助逻辑
- `scripts/theme_resolver.py`：主题、配色 token、recommendation 来源与 anchor 索引摘要诊断
